...
pub fn write_document(out_folder: &Path, file_name: &str, doc: &Document) -> Result<(), String> {
    let project_dirs = ProjectDirs::from("", "training_assistant", "training_assistant").unwrap();
    let temp_dir = project_dirs.cache_dir().join("documents");

    println!("temp dir: {:?}", temp_dir);

    let tex_path = temp_dir.join(format!("{}.tex", file_name));
    let pdf_path = temp_dir.join(format!("{}.pdf", file_name));
    let dest_path = out_folder.join(format!("{}.pdf", file_name));
    ...
    cmd.arg(format!("-output-directory={}", temp_dir.display()))
        .arg(tex_path);
    ...
    println!("copying generated pdf at {:?} to {:?}", pdf_path, dest_path);
