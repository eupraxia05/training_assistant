...
pub fn derive_row(item: TokenStream) -> TokenStream {
    ...
    let generated = generate_table_row_impl(&parsed_struct, &parsed_fields);
...
fn generate_table_row_impl(
    ...
    let setup_fn_definition = generate_table_row_setup_fn_definition(parsed_fields);
...
fn generate_table_row_setup_fn_definition(parsed_fields: &venial::NamedFields)
    -> proc_macro2::TokenStream
{ 
...
fn generate_table_row_from_table_row_fn_definition(parsed_fields: &venial::NamedFields)
    -> proc_macro2::TokenStream
{
    let mut from_table_row_fn_body = proc_macro2::TokenStream::new();
    ...
    for (field, _) in parsed_fields.fields.iter() {
        ...
        from_table_row_fn_body.extend(s.parse::<proc_macro2::TokenStream>().expect("fail 3"));
    }
    ...
    self_str += "})";
    from_table_row_fn_body.extend(self_str.parse::<proc_macro2::TokenStream>().unwrap());
